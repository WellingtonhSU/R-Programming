...
for( i in startPage:endPage)
{
  ...
  if(urlExists)
  {
    html = getURL(pttURL, ssl.verifypeer = FALSE)     #Crawl the full HTML file
    ...
    alldata = rbind(alldata, tempdata)                #Assign data into AllData
    
  }
}

allDate = levels(alldata$date)                        #Gather data with same levels
